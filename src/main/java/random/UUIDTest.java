...
/**
 *  UUID生成测试，32位，24位（Base64编码），16位（32位前半后半异或）
 * @author angilin
 *
 */
public class UUIDTest {
	...
	public static void main(String[] args) {
		...
		c.set(Calendar.YEAR, 3411);
		System.out.println(c.getTimeInMillis());

		c.setTimeInMillis(Integer.MAX_VALUE);
		System.out.println(c.get(Calendar.YEAR));*/
		// UUID第一部分8位是时间，不过和timeinMills无关
		// 2的8次方也就是int的最大值，也留在了1970年
		...
		/*
		 * //如果byte[]没有转String，输出类似[B@4b28899a System.out.println(new
		 * Base64().encode(uuid.toString().replaceAll("-", "").getBytes()));
		 * //如果没有把uuid作16进制转String操作，转出的字符串会更长（等于把每个char转byte了）
		 * System.out.println(new
		 * Base64().encodeToString(uuid.toString().replaceAll("-",
		 * "").getBytes()));
		 */
